...
def matchChildParent(repoPath, parentBuginstances, childBuginstances, parentCommit, childCommit, githubUrl, jClass):
    ...
    paCandidates, chCandidates, candidatesList = findSnippetCandidates(untrackedParent, untrackedChild, diffMap,
                                                                        parentChangedPaths,parentCommit, childCommit)
...
###Todo:take refactoring location matching
def findRefactoringLocationCandidates(untrackedParent,untrackedChild,diffMap, parentChangedPaths,refactoringInfo):
    paCandidates, chCandidates, candidatesList = [], [], []
...
if __name__ == "__main__":
    ...
    parentCommits = ["09cf57101d8b961a32ebc1adc3b1b093c00278b1"]
    childCommits = ["717a545baa5bbc9ae9c78a29a313898abf2403a7"]
    ...
    for i in range(len(parentCommits)):
        ...
        #                         parentCommits[i][0:7] + "_" + childCommits[i][0:7] + "_gone_spotbugs_hungarian.csv")
        # dataframe.to_csv(
        #     savePath,
        #     index=False,
        #     sep=',')
        ...
        #      "start": startSetParent, "end": endSetParent})
        # savePath = os.path.join(saveResultsPath,
        #                         parentCommits[i][0:7] + "_" + childCommits[i][0:7] + "_new_spotbugs_hungarian.csv")
        # dataframe.to_csv(
        #     savePath,
        #     index=False,
        #     sep=',')
        ...
        childBuginstances = PMD(parentPMDFileName)
        ...
        dataframe = pd.DataFrame(
            {'Bug': bugAbbvSetParent, 'category': categorySetParent, "class name": classSetParent,
             ...
             "start": startSetParent, "end": endSetParent})
        savePath = os.path.join(saveResultsPath,
                                parentCommits[i][0:7] + "_" + childCommits[i][0:7] + "_gone_pmd_hungarian.csv")
        dataframe.to_csv(
            savePath,
            index=False,
            sep=',')
        ...
        dataframe = pd.DataFrame(
            {'Bug': bugAbbvSetParent, 'category': categorySetParent, "class name": classSetParent,
             ...
             "start": startSetParent, "end": endSetParent})
        savePath = os.path.join(saveResultsPath,
                                parentCommits[i][0:7] + "_" + childCommits[i][0:7] + "_new_pmd_hungarian.csv")
        dataframe.to_csv(
            savePath,
            index=False,
            sep=',')
